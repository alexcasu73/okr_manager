...
const Sidebar: React.FC<SidebarProps> = ({ currentView, onChangeView }) => {
  ...
  return (
    <div className="w-64 h-screen bg-white dark:bg-slate-900 flex flex-col p-5 border-r border-slate-100 dark:border-slate-800 fixed left-0 top-0 z-10 hidden lg:flex transition-colors duration-300 overflow-y-auto">
      ...
      <button
        onClick={toggleTheme}
        className="flex items-center gap-3 px-3.5 py-2.5 text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition-colors mb-2 text-sm font-medium"
      >
      ...
      <button
        ...
        className="flex items-center gap-3 px-3.5 py-2.5 text-red-500 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl transition-colors text-sm font-medium"
      >
